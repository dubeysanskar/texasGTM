...
export default function LoginPage() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (otpRes.ok) {
        ...
        // If OTP fails (no SMTP), just proceed with login
  ...
  const resendOtp = async () => {
    ...
    if (res.ok) setSuccess('New code sent!');
    ...
    <div className="login-page">
      <div className="login-card">
        <div className="login-logo">
          ...
          <p>{mode === 'login' ? 'Sign in to your workspace' : mode === 'register' ? 'Create your account' : mode === 'forgot' ? 'Reset your password' : 'Enter verification code'}</p>
        </div>
        ...
        {mode === 'otp' && (
          <form onSubmit={handleOtpVerify}>
            <p style={{ fontSize: '0.82rem', color: '#475569', textAlign: 'center', marginBottom: 16 }}>We sent a 6-digit code to <strong>{otpEmail}</strong></p>
            <div className="form-group">
              <label>Verification Code</label>
              <input className="form-input" type="text" placeholder="000000" value={otp} onChange={e => setOtp(e.target.value.replace(/\D/g, '').slice(0, 6))} required maxLength={6} style={{ textAlign: 'center', fontSize: '1.3rem', letterSpacing: 8, fontWeight: 700 }} />
            </div>
            ...
            <div style={{ textAlign: 'center', marginTop: 12, fontSize: '0.78rem' }}>
              ...
              <button type="button" onClick={() => { setMode('login'); setOtp(''); setError(''); setSuccess(''); }} style={{ background: 'none', border: 'none', color: '#94a3b8', cursor: 'pointer' }}>Back to Login</button>
            </div>
          </form>
        )}
        ...
        <div style={{ textAlign: 'center', marginTop: 16, fontSize: '0.82rem', color: '#94a3b8' }}>
          {mode === 'login' && <>Don't have an account? <button onClick={() => { setMode('register'); setError(''); setSuccess(''); }} style={{ color: '#6366f1', background: 'none', border: 'none', fontWeight: 600, cursor: 'pointer' }}>Sign Up</button></>}
