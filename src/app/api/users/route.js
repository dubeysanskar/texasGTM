...
export async function POST(request) {
  const user = getUserFromRequest(request);
  if (!user || !isAdmin(user.role)) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
  const { name, email, password, role, project_id, project_role } = await request.json();
  if (!name || !email || !password) return NextResponse.json({ error: 'Name, email, password required' }, { status: 400 });
  const finalRole = role || 'staff';
  // Non-super-admins must be mapped to a project; super admins have access to all projects
  if (finalRole !== 'super_admin' && !project_id) return NextResponse.json({ error: 'Please assign a project (required for all roles except Super Admin)' }, { status: 400 });
  ...
  if (finalRole !== 'super_admin' && project_id) {
    const project = await queryOne('SELECT id FROM gtm_projects WHERE id = $1 AND is_active = true', [project_id]);
    if (project) {
      await query(
